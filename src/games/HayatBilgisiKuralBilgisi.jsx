...
import { Star, Trophy, RotateCcw, HelpCircle, ShieldCheck } from "lucide-react";
...
// ---- GERÇEK SORU BANKASI ----
// Pratik Modu'nun ustalık bazlı seviye motoru (bkz. PratikModuMatematik.jsx),
// içerik "kural_bilgisi" kayıtları (okul kuralları, güvenlik, aile/toplum,
// çevre) - Doğru/Yanlış cevap. Doğa Gözlemi'nin (görsel arama) ve Zaman
// Sırası'nın (sıralama) yanına "kural/değer bilgisi" becerisini ekliyor -
// bkz. devir-teslim Bölüm 4 madde 2. Sınıf bazlı: `sinif` prop'una göre
// doğru banka seçilir.
...
function sorulariHazirla(sinif) {
  ...
  return banka.sorular
    .filter((s) => s.soru_tipi === "kural_bilgisi")
    .map((s) => ({
      id: s.id,
      seviye: s.seviye,
...
// geri düş. Süre baskısı yok - Pratik Modu ile aynı motor.
...
export default function HayatBilgisiKuralBilgisi({ onExit, onComplete, sinif = 1 } = {}) {
  ...
  return (
    <div className="kural-root">
      <style>{`
        ...
        .option-btn.dogru { background: var(--grass-dark); }
        .option-btn.yanlis { background: var(--red); }
        .option-btn.selected-correct { outline: 3px solid #FFC93C; outline-offset: -3px; transform: scale(1.03); }
        .option-btn.wrong-pick { outline: 3px solid #8A2E29; outline-offset: -3px; }
      ...
      <div className="top-row">
        <h1 className="brand"><ShieldCheck size={22} color="#5AB4E0" /> Kural Bilgisi</h1>
      ...
      <div className="options-row">
        {["Doğru", "Yanlış"].map((opt) => {
          let cls = `option-btn ${opt === "Doğru" ? "dogru" : "yanlis"}`;
          if (feedback === "wrong" && opt === wrongPick) cls += " wrong-pick";
          if (feedback === "correct" && opt === current.dogru_cevap) cls += " selected-correct";
          if (feedback === "wrong" && ayniSoruDenemeSayisi >= 2 && opt === current.dogru_cevap) cls += " selected-correct";
          return (
            <button key={opt} className={cls} onClick={() => handlePick(opt)}>
              <span className="option-emoji">{opt === "Doğru" ? "✅" : "❌"}</span>
              {opt}
            </button>
          );
        })}
      ...
      {showTutorial && (
        <div className="tutorial-overlay">
          ...
          <div className="finish-title">Doğru mu Yanlış mı?</div>
          <div className="tutorial-steps">
            <div className="tutorial-step">
              <span className="tutorial-step-icon">📖</span>
              <span>Cümleyi oku, doğru mu yanlış mı düşün</span>
            </div>
            <div className="tutorial-step">
              <span className="tutorial-step-icon">🔥</span>
